"""Batch process all 53 frames x 2 characters with v17 pipeline.
Outputs PNG frames in assets/final/roach/ and assets/final/gorilla/."""
import os, math, time
import numpy as np
...
os.makedirs(os.path.join(OUT, "roach"), exist_ok=True)
os.makedirs(os.path.join(OUT, "gorilla"), exist_ok=True)
...
def build_frame(face_name, body_path, body_head_xy, head_target_h, frame):
    ...
    # Upright: tilt > 0 means face tilted CW, so rotate CCW by +tilt
...
for frame in range(1, 54):
    for name, body, head_xy, th, tag in [
        ("left", ROACH_BODY, (179, 175), 230, "roach"),
        ("right", GOR_BODY, (150, 75), 110, "gorilla"),
    ]:
        body_img = build_frame(name, body, head_xy, th, frame)
        if body_img is None:
            print(f"frame {frame} {tag}: NO FACE")
            continue
        body_img.save(os.path.join(OUT, tag, f"f_{frame:03d}.png"))
    if frame % 10 == 0:
        print(f"frame {frame}/53 done ({time.time()-t0:.1f}s)")

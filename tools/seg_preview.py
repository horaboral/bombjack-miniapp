"""Composite segmented test frames over a checkerboard to inspect alpha."""
import os
...
for name in ("left", "right"):
    for i in (0, 27, 53):
        fg = Image.open(os.path.join(out, f"{name}_{i:03d}_test.png")).convert("RGBA")
        bg = checker(fg.width, fg.height)
        comp = Image.alpha_composite(bg.convert("RGBA"), fg).convert("RGB")
        comp.save(os.path.join(out, f"{name}_{i:03d}_check.jpg"), quality=85)
print("previews done")
